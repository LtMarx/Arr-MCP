...
function makeClient(baseUrlKey, apiKeyKey) {
...
function requireService(service, name) {
    if (!service)
        throw new Error(`${name} is not configured. Set ${name.toUpperCase()}_URL and ${name.toUpperCase()}_API_KEY environment variables.`);
    return service;
}
function ok(data) {
...
server.tool("radarr_search_movies", "Search for movies to add to Radarr", { term: z.string().describe("Search term (movie title)") }, async ({ term }) => {
    const results = await requireService(radarr, "Radarr").searchMovies(term);
    return ok(results);
});
server.tool("radarr_add_movie", "Add a movie to Radarr by TMDB ID", {
...
server.tool("radarr_delete_movie", "Delete a movie from Radarr", {
...
server.tool("sonarr_search_series", "Search for a TV series to add to Sonarr", { term: z.string().describe("Search term (series title)") }, async ({ term }) => {
    const results = await requireService(sonarr, "Sonarr").searchSeries(term);
    return ok(results);
});
server.tool("sonarr_add_series", "Add a TV series to Sonarr by TVDB ID", {
...
server.tool("sonarr_delete_series", "Delete a series from Sonarr", {
...
server.tool("sonarr_get_episodes", "Get episodes for a series", { seriesId: z.number().describe("Sonarr series ID") }, async ({ seriesId }) => {
...
server.tool("lidarr_search_artists", "Search for an artist to add to Lidarr", { term: z.string().describe("Artist name to search") }, async ({ term }) => {
    const results = await requireService(lidarr, "Lidarr").searchArtists(term);
    return ok(results);
});
server.tool("lidarr_add_artist", "Add an artist to Lidarr by MusicBrainz ID", {
...
server.tool("lidarr_get_albums", "Get albums, optionally filtered by artist", { artistId: z.number().optional().describe("Filter by Lidarr artist ID") }, async ({ artistId }) => {
...
server.tool("readarr_search_authors", "Search for an author to add to Readarr", { term: z.string().describe("Author name to search") }, async ({ term }) => {
    const results = await requireService(readarr, "Readarr").searchAuthors(term);
    return ok(results);
});
server.tool("readarr_add_author", "Add an author to Readarr by Goodreads/foreign ID", {
...
server.tool("readarr_get_books", "Get books, optionally filtered by author", { authorId: z.number().optional().describe("Filter by Readarr author ID") }, async ({ authorId }) => {
    const books = await requireService(readarr, "Readarr").getBooks(authorId);
    return ok(books);
});
server.tool("readarr_search_books", "Search for a book in Readarr", { term: z.string().describe("Book title or ISBN to search") }, async ({ term }) => {
...
server.tool("prowlarr_search", "Search across all Prowlarr indexers", {
